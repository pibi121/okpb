#!/usr/bin/env python3
"""Test downsample-blur of reference before img2img: kills facial identity, keeps pose/silhouette, no detector needed."""
...
def build(blur_px, denoise, seed, prefix):
    return {
        ...
        "312": {"class_type": "PreviewImage", "inputs": {"images": ["331", 0]}},
...
TESTS = [
    (64, 0.6, 778899001, "Flux2_BLUR64_D060"),
    (32, 0.6, 778899002, "Flux2_BLUR32_D060"),
    (32, 0.75, 778899003, "Flux2_BLUR32_D075"),
]
...
req = urllib.request.Request(
    ...
    data=json.dumps({"prompt": prompt, "client_id": "blur_i2i_test"}).encode(),
    ...
    for i in range(60):
        ...
        if prompt_id in h:
            ...
            if completed is not None:
                ...
                print("OUTPUTS", json.dumps(outputs)[:1200])
                if status_str == "error":
...
def main():
    ...
    for blur_px, denoise, seed, prefix in TESTS:
        print(f"\n=== {prefix} (blur_px={blur_px}, denoise={denoise}) ===")
        p = build(blur_px, denoise, seed, prefix)
        sftp = client.open_sftp()
        with sftp.file("/work/_blur_i2i_run.py", "w") as f:
            f.write(REMOTE_TPL % repr(p))
        sftp.close()
        stdin, stdout, stderr = client.exec_command("python3 /work/_blur_i2i_run.py", timeout=300)
