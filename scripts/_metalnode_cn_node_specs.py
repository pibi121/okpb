#!/usr/bin/env python3
"""Dump full input/output spec for specific nodes needed to build the ControlNet workflow."""
...
for n in names:
    ...
    info = data[n]
    print(f"=== {n} ===")
    print("inputs:", json.dumps(info.get("input", {}), indent=None)[:1500])
...
def main():
    ...
    with sftp.file("/work/_node_specs.py", "w") as f:
        f.write(REMOTE)
    sftp.close()

    stdin, stdout, stderr = client.exec_command("python3 /work/_node_specs.py", timeout=40)
