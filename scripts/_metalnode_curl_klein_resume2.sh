...
# Prefer live DNS; fall back to IPs that already worked for this host
...
[ -z "$HF_IP" ] && HF_IP=52.222.136.117
# DO NOT reuse CAS IP for us.aws.cdn — cert mismatch
[ -z "$CDN_IP" ] && CDN_IP=15.236.34.207
[ -z "$CAS_IP" ] && CAS_IP=15.236.34.207

log "RESUME2 HF=$HF_IP CDN=$CDN_IP CAS=$CAS_IP part=$(stat -c%s "$PART" 2>/dev/null || echo 0)"
...
log "CURL_RESUME2_START"
curl -4 -L --retry 80 --retry-all-errors --retry-delay 3 \
  -C - \
  "${RESOLVE_ARGS[@]}" \
  --connect-timeout 30 \
  -o "$PART" \
