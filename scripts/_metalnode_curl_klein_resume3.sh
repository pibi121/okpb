...
log "RESUME3 HF=$HF_IP CDN=$CDN_IP CAS=$CAS_IP part=$(stat -c%s "$PART" 2>/dev/null || echo 0)"
...
# Probe redirect
log "PROBE"
curl -4 -sI -L --max-redirs 10 "${RESOLVE_ARGS[@]}" "$URL" 2>&1 | tr -d '\r' | grep -iE '^(HTTP/|location:)' | tee -a "$LOG" | tail -20

log "CURL_RESUME3_START"
