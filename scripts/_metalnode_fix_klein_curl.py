...
SCR = Path(__file__).with_name("_metalnode_curl_klein_verify.sh")
...
def main():
    c = connect()
    sftp = c.open_sftp()
    sftp.put(str(SCR), "/tmp/curl_klein_verify.sh")
    sftp.close()
    out, _ = run(
        c,
        "nohup bash /tmp/curl_klein_verify.sh > /work/REDOWNLOAD_KLEIN_CURL.nohup 2>&1 & echo PID:$!; sleep 3; cat /work/REDOWNLOAD_KLEIN_CURL.log",
        timeout=30,
    )
    ...
    for i in range(240):
        ...
        out, _ = run(
            c,
            "tail -12 /work/REDOWNLOAD_KLEIN_CURL.log; ls -lh /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors* 2>/dev/null; pgrep -c curl || true",
            timeout=30,
        )
        ...
        if "READY" in out and EXPECT[:16] in out:
            ...
            break
        if any(x in out for x in ("SHA_MISMATCH", "SIZE_MISMATCH")) and "curl_exit" in out:
            # only fail after curl finished
            if "READY" not in out:
...
if ! curl -s -m 3 http://127.0.0.1:8188/system_stats >/dev/null; then
  ...
  for i in $(seq 1 20); do sleep 2; curl -s -m 2 http://127.0.0.1:8188/system_stats >/dev/null && break; done
fi
python3 - <<'PY'
