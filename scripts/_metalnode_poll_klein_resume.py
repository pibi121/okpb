...
SCR = Path(__file__).with_name("_metalnode_curl_klein_resume.sh")
...
def main():
    c = connect()
    sftp = c.open_sftp()
    sftp.put(str(SCR), "/tmp/curl_klein_resume.sh")
    sftp.close()
    out, _ = run(
        c,
        "nohup bash /tmp/curl_klein_resume.sh > /work/REDOWNLOAD_KLEIN_CURL.nohup 2>&1 & echo PID:$!; sleep 4; tail -20 /work/REDOWNLOAD_KLEIN_CURL.log | tr '\\r' '\\n' | tail -20",
        timeout=30,
    )
    print(out, flush=True)

    for i in range(360):  # up to 3h
        time.sleep(30)
        out, _ = run(
            c,
            "tail -c 800 /work/REDOWNLOAD_KLEIN_CURL.log | tr '\\r' '\\n' | tail -8; ls -lh /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors* 2>/dev/null; pgrep -c curl || true; grep -E 'READY|SHA_MISMATCH|SIZE_MISMATCH|curl_exit' /work/REDOWNLOAD_KLEIN_CURL.log | tail -5",
            timeout=30,
        )
        ...
        if "SHA_MISMATCH" in out or "SIZE_MISMATCH" in out:
            # only fail if curl finished
            if "curl_exit=" in out and "READY" not in out:
                # check if still downloading
                if "pgrep" not in out and out.strip().endswith("0"):
