...
SCR = Path(__file__).with_name("_metalnode_curl_klein_resume3.sh")
...
def main():
    c = connect()
    sftp = c.open_sftp()
    sftp.put(str(SCR), "/tmp/curl_klein_resume3.sh")
    sftp.close()
    out, _ = run(
        c,
        "nohup bash /tmp/curl_klein_resume3.sh > /work/REDOWNLOAD_KLEIN_CURL.nohup 2>&1 & echo PID:$!; sleep 12; grep -E 'RESUME3|CURL_RESUME3|PROBE|HTTP/|location:|Resuming|SSL|curl_exit3|READY' /work/REDOWNLOAD_KLEIN_CURL.log | tail -30; ls -lh /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors.part; pgrep -a curl | head -3",
        timeout=60,
    )
    ...
    for i in range(360):
        ...
        out, _ = run(
            c,
            "tail -c 600 /work/REDOWNLOAD_KLEIN_CURL.log | tr '\\r' '\\n' | tail -6; ls -lh /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors* 2>/dev/null; pgrep -c curl || echo 0; grep -E 'READY|SHA_MISMATCH3|SIZE_MISMATCH3|curl_exit3' /work/REDOWNLOAD_KLEIN_CURL.log | tail -3",
            timeout=30,
        )
        ...
        if "READY" in out and EXPECT[:16] in out:
            ...
            break
        if "curl_exit3=" in out and "READY" not in out:
            # finished without ready?
            curl_count = out.strip().splitlines()
            # if no curl and exit logged after RESUME3
            if any(x.startswith("curl_exit3=") and not x.endswith("=0") for x in out.splitlines()):
                # still downloading if curl > 0
                pass
            if "\n0\n" in ("\n" + out) or out.rstrip().endswith("\n0"):
                if "SIZE_MISMATCH3" in out or "SHA_MISMATCH3" in out or (
                    "curl_exit3=" in out and "curl_exit3=0" not in out
                ):
                    # check latest exit
                    lines = [l for l in out.splitlines() if "curl_exit3=" in l]
                    if lines and not lines[-1].endswith("=0") and "READY" not in out:
...
if ! curl -s -m 3 http://127.0.0.1:8188/system_stats >/dev/null; then
  ...
  for i in $(seq 1 30); do sleep 2; curl -s -m 2 http://127.0.0.1:8188/system_stats >/dev/null && break; done
fi
python3 - <<'PY'
