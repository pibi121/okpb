...
SCR = Path(__file__).with_name("_metalnode_curl_klein_resume4.sh")
...
def main():
    sftp_c = connect()
    sftp = sftp_c.open_sftp()
    sftp.put(str(SCR), "/tmp/curl_klein_resume4.sh")
    sftp.close()
    sftp_c.close()

    print(
        run_once(
            "nohup bash /tmp/curl_klein_resume4.sh > /work/REDOWNLOAD_KLEIN_CURL.nohup 2>&1 & echo PID:$!; sleep 8; grep -E 'RESUME4|CURL_RESUME4|Resuming|SSL|curl_exit4' /work/REDOWNLOAD_KLEIN_CURL.log | tail -15; ls -lh /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors.part; pgrep -a curl | head -2"
        ),
        flush=True,
    )

    for i in range(300):
        time.sleep(45)  # gentler on sshd
        out = run_once(
            "stat -c%s /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors.part 2>/dev/null || echo 0; pgrep -c curl || echo 0; grep -E 'READY|curl_exit4|SHA_MISMATCH4|SIZE_MISMATCH4' /work/REDOWNLOAD_KLEIN_CURL.log | tail -4; tail -c 200 /work/REDOWNLOAD_KLEIN_CURL.log | tr '\\r' '\\n' | tail -2"
        )
        ...
        if "READY" in out and EXPECT[:16] in out:
            ...
            break
        if "curl_exit4=" in out and "READY" not in out:
            # stalled exit?
            if "\n0\n" in ("\n" + out) or out.strip().endswith("0"):
                lines = [l for l in out.splitlines() if "curl_exit4=" in l]
                if lines and lines[-1] != "curl_exit4=0":
                    print("DOWNLOAD_FAIL_RETRY", flush=True)
                    run_once(
                        "nohup bash /tmp/curl_klein_resume4.sh > /work/REDOWNLOAD_KLEIN_CURL.nohup 2>&1 & echo RESTARTED:$!"
                    )
    else:
