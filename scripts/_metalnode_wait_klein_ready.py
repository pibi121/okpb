#!/usr/bin/env python3
"""Reconnect each poll; wait for READY then test-gen."""
...
echo PART:$(stat -c%s /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors.part 2>/dev/null || echo 0)
echo DEST:$(stat -c%s /work/ComfyUI/models/diffusion_models/flux-2-klein-9b-fp8.safetensors 2>/dev/null || echo 0)
pgrep -c curl || echo CURL:0
grep -E 'READY|SHA_MISMATCH3|SIZE_MISMATCH3|curl_exit3|RESUME3' /work/REDOWNLOAD_KLEIN_CURL.log | tail -8
tail -c 350 /work/REDOWNLOAD_KLEIN_CURL.log | tr '\\r' '\\n' | tail -3
...
def run(cmd, timeout=60):
    for attempt in range(5):
        try:
            ...
            print(f"ssh_retry {attempt}: {e}", flush=True)
            time.sleep(5)
...
def main():
    ...
    for i in range(240):
        ...
        if "curl_exit3=" in out and "READY" not in out:
            # if no curl and failed
            if "CURL:0" in out or "\n0\n" in out:
                lines = [l for l in out.splitlines() if "curl_exit3=" in l]
                if lines and not lines[-1].endswith("=0"):
                    if "SIZE_MISMATCH3" in out or "SHA_MISMATCH3" in out or True:
                        # only fail if exit != 0
                        if not lines[-1].endswith("=0"):
...
for i in range(180):
    ...
    if pid in h:
        ...
        print('DONE', list(outs.keys())); break
    time.sleep(2)
else:
