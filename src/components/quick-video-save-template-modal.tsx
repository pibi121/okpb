...
import type { TemplateCategory } from "@/lib/quick-video-template";

const JUICE_PRESETS = [0, 60, 80, 100, 120, 150] as const;

export function QuickVideoSaveTemplateModal({
  open,
  sourceRunId,
  defaultTitle,
  onClose,
  ...
  async function save() {
    ...
    setError("");
    try {
      const res = await fetch("/api/peach/quick-video/templates", {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#161618] p-5 shadow-xl">
        <h3 className="text-lg font-medium">Сохранить как шаблон</h3>
        <p className="mt-1 text-xs text-zinc-500">
          Рецепт сцены без ваших identity-фото. Покупка шаблона — разовая, генерации
            ...
            >
              {JUICE_PRESETS.filter((p) => p > 0).map((p) => (
                <option key={p} value={p}>
                  {p} кр. (~${(p * 0.01).toFixed(2)})
                </option>
              ))}
            </select>
          </label>
        ) : (
          <p className="mt-2 text-xs text-zinc-600">Бесплатный — доступен всем сразу.</p>
        )}
